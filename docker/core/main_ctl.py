...
# LED_LINES 기본값 2: 설정 없을 때 고도화 이전 2라인 표시 방식 유지 (하위 호환)
# 1라인 전광판은 .env 에 LED_LINES=1 명시 필요
...
def encode_to_protocol(message, default_template, color="00", font="00", weight="01", eff="000000000000", ysz="2",
    ...
    if not message:
        ...
        # 2라인 전광판: LNE=1, LNE=2 모두 동일 메시지 — 시계 포맷과 동일하게 $f$c 순서, $w/$A00 제거
        formatted_message = (
...
def update_count():
    ...
    try:
        ...
        radar_active = data.get('radar_active', False)
        logging.info(f"[update_count] count={detected_people_count} source={source} radar_active={radar_active}")
...
async def send_stomp_message(destination, message, url):
    if ENV_TYPE == 'dev':
        logging.info(f"send_stomp_message: dev 모드 - 전송 스킵 {destination}: {message}")
        return
    try:
