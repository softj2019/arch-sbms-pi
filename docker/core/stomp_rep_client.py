...
if _STATION_TYPE != "smartpole" and _CAMERA_TYPE != "usb" and camera_ip:
    ...
    logging.info(f"stomp_rep_client: IP 카메라 연결 생략 (STATION_TYPE={_STATION_TYPE}, CAMERA_TYPE={_CAMERA_TYPE})")

async def run_handle_power_action(device, action=None, override_duration_sec=None):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, handle_power_action, device, action, override_duration_sec)
...
def handle_power_action(device, action=None, override_duration_sec=None):
    payload = {"device": device, "duration": 1}
    if action is not None:
        payload["action"] = action
    if override_duration_sec is not None:
        payload["override_duration_sec"] = override_duration_sec
    try:
    ...
    while True:
        try:
            async with websocket_connection(primary_url=url, log_context="stomp_req_client") as websocket:
                ...
                reconnect_delay = 1  # 연결 성공 시 백오프 초기화
                ...
                subscribe_cv_stream_frame = create_stomp_subscribe_frame(destination="/topic/cv/stream", subscription_id="4")
                await websocket.send(subscribe_cv_stream_frame)
                ...
                # 3. 서버 메시지 수신 처리
                RECV_TIMEOUT_SEC = int(os.getenv("STOMP_REP_RECV_TIMEOUT_SEC", "30"))
                while True:
                    try:
                        server_message = await asyncio.wait_for(websocket.recv(), timeout=RECV_TIMEOUT_SEC)
                        report_ws_alive("stomp_rep_client")
                        ...
                        # 메시지 없이 RECV_TIMEOUT_SEC 경과 → ping으로 좀비 연결 감지
                        try:
                            await websocket.ping()
                            logging.debug("stomp_req_client: ping OK (연결 유지)")
                        except Exception:
                            logging.warning("stomp_req_client: ping 실패 - 좀비 연결 감지, 재연결")
                            raise websockets.ConnectionClosedError(None, None)
                    # ConnectionClosed는 잡지 않고 외부 except로 전파 → 재연결

        except (websockets.ConnectionClosed, websockets.ConnectionClosedError,
                websockets.ConnectionClosedOK, ConnectionRefusedError, ConnectionError) as e:
            ...
            logging.error(f"stomp_req_client: 예기치 못한 오류: {e}. {reconnect_delay}초 후 재연결 시도")
            await asyncio.sleep(reconnect_delay)
