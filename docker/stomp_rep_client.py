...
async def run_handle_power_action(device, action=None):
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, handle_power_action, device, action)
...
def handle_power_action(device, action=None):
                ...
                if "\n\n" in server_message:
                    ...
                    if terminal_id == TERMINAL_ID:
                        ...
                        #     led_display_send_message(message)
                            ...
                            power_action = message.get("action")
                            logging.info(f"Executing power action: device={device} action={power_action}")
                            # 이전에 실행 중인 power 액션 작업이 없다면 새 작업 생성
                            if power_action_task is None or power_action_task.done():
                                power_action_task = asyncio.create_task(run_handle_power_action(device, power_action))
                            else:
